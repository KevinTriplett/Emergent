...
class EmergeSpider < Kimurai::Base
  ...
  def sign_in(response, url:, data: {})
    ...
    EmergeSpider.logger.debug "SIGNING IN"
    ...
    EmergeSpider.logger.debug "SUCCESS!"
  end
  ...
  def wait_while(css)
    ...
    while response_has(css) || i < 0
      EmergeSpider.logger.debug "WAITING WHILE #{css} ..."
      sleep 1
      i -= 1
    end
    EmergeSpider.logger.debug "NEVER WENT AWAY!" if response_has(css)
  end
  ...
  def wait_until(css)
    ...
    until response_has(css) || i < 0
      EmergeSpider.logger.debug "WAITING UNTIl #{css} ..."
      sleep 1
      i -= 1
    end
    EmergeSpider.logger.debug "COULD NOT FIND IT!" unless response_has(css)
  end
end
