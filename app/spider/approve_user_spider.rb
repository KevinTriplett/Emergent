...
class ApproveUserSpider < EmergeSpider
  ...
  def parse(response, url:, data: {})
    NewUserSpider.logger.debug "APPROVING USER WITH EMAIL #{ApproveUserSpider.user_email}"
    sign_in
    ...
    # browser.save_screenshot
    NewUserSpider.logger.debug "COMPLETED SUCCESSFULLY"
  end

  def approve_user(response, url:, data: {})
    NewUserSpider.logger.debug "ATTEMPTING TO FIND AND CLICK APPROVE FOR USER WITH EMAIL #{ApproveUserSpider.user_email}"
    ...
    NewUserSpider.logger.debug "LOOKING FOR #{css}"
